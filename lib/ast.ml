...
| Unit
| Pair of pterm * pterm                (* Nouveau : produit *)
| Fst of pterm                         (* Nouveau : projection gauche *)
| Snd of pterm                         (* Nouveau : projection droite *)
| Inl of pterm                         (* Nouveau : injection gauche (somme) *)
| Inr of pterm                         (* Nouveau : injection droite (somme) *)
| Case of pterm * (string * pterm) * (string * pterm)
...
let rec print_term (t : pterm) : string =
    ...
    | Fst t -> "fst(" ^ print_term t ^ ")"
    | Snd t -> "snd(" ^ print_term t ^ ")"
    | Inl t -> "inl(" ^ print_term t ^ ")"
    | Inr t -> "inr(" ^ print_term t ^ ")"
...
let  alphaconv (t : pterm) : pterm =
    ...
    let rec alphaconv_aux (p_aux : pterm) (var_mapping : (string * string) list) : pterm =
        ...
        | Unit -> Unit
        | Pair (t1, t2) -> Pair (alphaconv_aux t1 var_mapping, alphaconv_aux t2 var_mapping)
        | Fst t -> Fst (alphaconv_aux t var_mapping)
        | Snd t -> Snd (alphaconv_aux t var_mapping)
        | Inl t -> Inl (alphaconv_aux t var_mapping)
        | Inr t -> Inr (alphaconv_aux t var_mapping)
        | Case (t, (x1, t1), (x2, t2)) ->
            let new_var1 = nouvelle_var () in
            let new_var2 = nouvelle_var () in
            let var_mapping1 = (x1, new_var1) :: var_mapping in
            let var_mapping2 = (x2, new_var2) :: var_mapping in
            Case (alphaconv_aux t var_mapping,
                  (new_var1, alphaconv_aux t1 var_mapping1),
                  (new_var2, alphaconv_aux t2 var_mapping2))
...
let rec substitution (x : string) (n : pterm) (t: pterm) : pterm =
    ...
    | Unit -> Unit
    | Pair (t1, t2) -> Pair (substitution x n t1, substitution x n t2)
    | Fst t1 -> Fst (substitution x n t1)
    | Snd t1 -> Snd (substitution x n t1)
    | Inl t -> Inl (substitution x n t)
    | Inr t -> Inr (substitution x n t)
    | Case (t, (x1, t1), (x2, t2)) ->
        Case (substitution x n t,
              (x1, if x = x1 then t1 else substitution x n t1),
              (x2, if x = x2 then t2 else substitution x n t2))
...
let valeur (t : pterm) : bool = 
    match t with 
    | Var _ | Abs _ | Nat _ | Unit | List _ | Pair _ | Inl _ | Inr _ -> true
    | _ -> false
    ...
    let rec ltr_cbv_step (t : pterm) : pterm option =
        match t with
        | Var _ | Abs _ | Nat _ | List _ | Unit | Inl _ | Inr _ -> None
        ...
        | Assign (e1, e2) -> 
            ...
            | None -> 
                ...
                | None -> None))
        | Pair (v1, v2) when valeur v1 && valeur v2 -> None
        | Pair (t1, t2) when not (valeur t1) -> (
            match ltr_cbv_step t1 with
        ...
        | Pair (t1, t2) -> (
            ...
            | None -> None)
        | Snd (Pair (_, v2)) when valeur v2 -> Some v2
        | Snd t1 -> (
            match ltr_cbv_step t1 with
            | Some t1' -> Some (Snd t1')
            | None -> None)
        | Inl v when valeur v -> None
        | Inr v when valeur v -> None
        | Case (Inl v, (x1, t1), _) when valeur v -> Some (substitution x1 v t1)
        | Case (Inr v, _, (x2, t2)) when valeur v -> Some (substitution x2 v t2)
        | Case (t, (x1, t1), (x2, t2)) -> (
            match ltr_cbv_step t with
            | Some t' -> Some (Case (t', (x1, t1), (x2, t2)))
            | None -> None)
